...
The client can be downloaded from [GitHub releases](https://github.com/heroiclabs/nakama-java/releases/latest). You can download "nakama-java-$version.jar" or "nakama-java-$version-all.jar" which includes a shadowed copy of all dependencies. <!-- If you use a build tool like Gradle you can skip the download and fetch it from the [central repository](#install-and-setup). -->
